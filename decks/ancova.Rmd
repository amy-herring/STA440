...
This set of notes is based on notes originally created by Peter Hoff.
...
![Ancova moth, or snout moth](images/ancovamoth.jpg){width=40%}
...
Hoff considers a subset of the NELS data that contains information on math scores of a random sample of 10th graders selected from a national sample of 685 large urban public schools. We plot the math scores $y_{ij}$ of the $n_j$ students in each school $j$, ranked by the average score.
...
Note in the prior plot that the highest estimated mean also has a very large variance. We may wish to use shrinkage estimation in order to stabilize estimates for schools in which few students provide data.  A random effects ANOVA model is given by $$y_{ij}=\mu+\alpha_j+\varepsilon_{ij},$$ where $\varepsilon_{ij} \sim N(0,\sigma^2)$ and $\alpha_j \sim N(0,\tau^2)$.
...
Among the schools, roughly 85% have positive slope estimates. The steepest slopes (positive and negative) tend to occur in the schools with smaller sample sizes.
...
Conditional independence: If I do know $\beta_j$, then knowing $y_{ij}$ does not give me any extra information about $y_{i'j}$, and they are independent. My information about $y_{ij} \perp y_{i'j}$ if I know $\beta_j$. (Note: a model can be extended to allow conditional dependence, e.g. over time within an individual)
...
mpar()
par(mfrow=c(1,2))
plot(BETA.OLS[,1],BETA.LME[,1],xlab="OLS intercept",ylab="HLM intercept")
abline(0,1) 
plot(BETA.OLS[,2],BETA.LME[,2],xlab="OLS slope",ylab="HLM slope")
abline(0,1) 
...
Certainly flp is doing something, though maybe we don't need that interaction term. 
