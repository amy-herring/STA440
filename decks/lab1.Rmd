...
```{r}
stansamps <- extract(stanfit)
which(colnames(descrim.modelmat) == "GenderMale")
stanfit #read off CI from beta[7] row
mean(stansamps$beta[,10] - stansamps$beta[,11] > 0)
rnorm(100000,stansamps$beta[,10] - stansamps$beta[,11],stansamps$sigma) %>% {.>0} %>% mean
(1-pnorm(0,stansamps$beta[,10] - stansamps$beta[,11],stansamps$sigma)) %>% mean
stansamps$sigma %>% {.>4000} %>% mean
```